...
const Register: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch('http://localhost:5002/api/auth/register/hospital', {
        method: 'POST',
        headers: {
      ...
      // Store user data and token in role-specific storage
      const storageKey = data.role === 'admin' ? 'user_admin' : 
                         data.role === 'donor' ? 'user_donor' : 
                         data.role === 'hospital' ? 'user_hospital' : 'user';
